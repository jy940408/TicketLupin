...
public class QuestionDao {
	...
	public List<QuestionVo> getQuestionList2(String keyword, String searchType, String qtype, String qstate, int page){
		...
		if(searchType.equals("����")){
			...
			sql = "SELECT @ROWNUM := @ROWNUM + 1 NUM, A.* FROM (SELECT QUESTION.*, MEMBER.MID, MEMBER.MNAME, MEMBER.MIDX MMIDX FROM QUESTION INNER JOIN MEMBER ON QUESTION.MIDX = MEMBER.MIDX WHERE QTITLE LIKE ? AND QTYPE LIKE ? AND QSTATE LIKE ? AND QDELYN = 'N' ORDER BY FIELD(QSTATE, '�Ϸ�', '���') DESC, QREGDATE ASC) A WHERE (@ROWNUM := 0) = 0 ORDER BY NUM LIMIT ?,?";
			
		}else if(searchType.equals("����")) {
			
			sql = "SELECT @ROWNUM := @ROWNUM + 1 NUM, A.* FROM (SELECT QUESTION.*, MEMBER.MID, MEMBER.MNAME, MEMBER.MIDX MMIDX FROM QUESTION INNER JOIN MEMBER ON QUESTION.MIDX = MEMBER.MIDX WHERE MNAME LIKE ? AND QTYPE LIKE ? AND QSTATE LIKE ? AND QDELYN = 'N' ORDER BY FIELD(QSTATE, '�Ϸ�', '���') DESC, QREGDATE ASC) A WHERE (@ROWNUM := 0) = 0 ORDER BY NUM LIMIT ?,?";
			
		}else{
			
			sql = "SELECT @ROWNUM := @ROWNUM + 1 NUM, A.* FROM (SELECT QUESTION.*, MEMBER.MID, MEMBER.MNAME, MEMBER.MIDX MMIDX FROM QUESTION INNER JOIN MEMBER ON QUESTION.MIDX = MEMBER.MIDX WHERE MID LIKE ? AND QTYPE LIKE ? AND QSTATE LIKE ? AND QDELYN = 'N' ORDER BY FIELD(QSTATE, '�Ϸ�', '���') DESC, QREGDATE ASC) A WHERE (@ROWNUM := 0) = 0 ORDER BY NUM LIMIT ?,?";
		}
